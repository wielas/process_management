...
std::string Lock::get_is_open_string()
{
	if (this->is_lock_open == 0) return "closed";
	else return "opened";
}

std::string Lock::get_name()
{
	return name;
};
...
//wyswietlanie kolejki procesow oczekujacyh
void Lock::print_processes_queue()
{
	std::cout << "Lista numerow procesow oczekujacych:\n";
	for (auto e : processes_queue)
	{
		std::cout << e->PID << ", ";
	}
	std::cout << "\n";
...
void Lock::set_who_closed_lock(PCB* process)
{
	if (!this->is_lock_open) // jesli zamek jest zamkniety (is_open = false) oznacza to, ze jakis proces go zamknal - tutaj ustalamy ktory proces zamyka zamek
	{
		...
		std::cout << "Proces o PID " << process->PID << " zamknal zamek o nazwie "; //praca krokowa - komunikat, ktory proces zamknal zamek
	}
...
void unlock_lock(Lock* lock, PCB* process)
{
	if (lock->get_is_open()) {
		...
		if (lock->get_processes_queue_size() > 0) //jezeli kolejka jest wieksza od 0 to
		{
			...
			std::cout << lock->get_name() << "\n";
			std::cout << "Proces o PID " << process->PID << " ponownie zamknal zamek. Jego stan, to ";
			if (process->status == 0) cout << " UNDECLARED" << endl;
			if (process->status == 1) cout << " RUNNING" << endl;
			if (process->status == 2) cout << " SLEEPING" << endl;
			if (process->status == 3) cout << " ZOMBIE" << endl;
			if (process->status == 4) cout << " INIT" << endl;
			if (process->status == 5) cout << " READY" << endl;
		...
		else // jezeli kolejka jest rowna 0
		{
			...
			else
			{
				...
				std::cout << "Proces przeszedl w stan ";
				if (process->status == 0) cout << " UNDECLARED" << endl;
				if (process->status == 1) cout << " RUNNING" << endl;
				if (process->status == 2) cout << " SLEEPING" << endl;
				if (process->status == 3) cout << " ZOMBIE" << endl;
...
void lock_lock(Lock* lock, PCB* process) //zamek zawsze jest wskaznikiem, proces tez (poniewaz zawsze musimy działac dynamicznie na danych)
{
	if (lock->get_is_open()) //jezeli zamek otwarty
	{
		...
		process->set_state(State::READY); // NIE WIEM CZY TO ZOSTAWIC!!!
		std::cout << lock->get_name() << "\n";
	...
	else // jak zamek jest zamkniety
	{
		if (lock->get_who_closed_lock() != process) // jezeli jest zamkniety przez inny proces, niz ten ktory chce zaczac dzialac na wpoldzielonych danych
		{
			...
			std::cout << "Zamek jest juz zamkniety. Proces o PID " << process->PID << " laduje w kolejce. Jego stan, to: ";
			if (process->status == 0) cout << " UNDECLARED" << endl;
			if (process->status == 1) cout << " RUNNING" << endl;
			if (process->status == 2) cout << " SLEEPING" << endl;
			if (process->status == 3) cout << " ZOMBIE" << endl;
		...
		{
			process->set_state(ZOMBIE);
			std::cout << "Blad systemu - Zamek jest juz zajety przez ten proces.\n";
			std::cout << "Proces przeszedl w stan ";
			if (process->status == 0) cout << " UNDECLARED" << endl;
			if (process->status == 1) cout << " RUNNING" << endl;
			if (process->status == 2) cout << " SLEEPING" << endl;
